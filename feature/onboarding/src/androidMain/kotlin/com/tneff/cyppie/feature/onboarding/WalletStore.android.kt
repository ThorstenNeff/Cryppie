...
/**
 * KAN-89/KAN-95 dedupe: persist to the shared `CiphertextStore.defaultFile()` (`<filesDir>/cyppie/
 * wallet.seed`) so the app-shell launch check (`walletExists`) reads the very file onboarding wrote.
 * Requires `AndroidStoragePaths.init(filesDir)` at startup (MainActivity).
 */
